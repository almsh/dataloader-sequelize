...
function getLoaderKey(modelName, schema) {
...
function loadersForModel(Model) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  const result = {
    bySingleAttribute: {}
  };
  result.bySingleAttribute[Model.primaryKeyAttribute] = createModelAttributeLoader(Model.schema(options.schema || ''), Model.primaryKeyAttribute, options);
  result.byId = result.byPrimaryKey = result.bySingleAttribute[Model.primaryKeyAttribute];

  const sequelelize = Model.sequelize;

  let externalAssociations = [];

  sequelelize.modelManager.forEachModel(model => {
    externalAssociations = externalAssociations.concat((0, _lodash.values)(model.associations).filter(association => association.target === Model));
  });

  (0, _lodash.values)(Model.associations).concat(externalAssociations).forEach(association => {
    if (association.associationType === 'BelongsTo') {
      const Target = association.target;
      if (association.targetKey !== Target.primaryKeyAttribute) {
        result.bySingleAttribute[association.targetKey] = createModelAttributeLoader(Target.schema(options.schema || ''), association.targetKey, options);
      }
    } else if (association.associationType === 'HasOne') {
      const Target = association.target;
      result.bySingleAttribute[association.foreignKey] = createModelAttributeLoader(Target.schema(options.schema || ''), association.foreignKey, options);
    }
  });
...
function shimModel(target) {
  ...
  _shimmer2.default.massWrap(target, (0, _helper.methods)(_sequelize2.default.version).findByPk, original => {
    return function batchedFindById(id) {
      ...
      let loader = options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.get(getLoaderKey(this.name, this._schema));

      if (!loader) {
        loader = loadersForModel(this, options);
        options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.set(getLoaderKey(this.name, this._schema), loader);
      }
      return _bluebird2.default.resolve(loader.byPrimaryKey.load(id)).then(rejectOnEmpty.bind(null, options));
    };
  });
}
...
function shimBelongsTo(target) {
  ...
  _shimmer2.default.wrap(target, 'get', original => {
    return function batchedGetBelongsTo(instance) {
      ...
      const schema = this.target._schema || options.schema;
      ...
      return _bluebird2.default.resolve().then(() => {
        ...
        let loaderForModel = options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.get(getLoaderKey(this.target.name, schema));

        if (!loaderForModel) {
          loaderForModel = loadersForModel(this.target, options);
          options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.set(getLoaderKey(this.target.name, schema), loaderForModel);
        }

        const loader = loaderForModel.bySingleAttribute[this.targetKey];
...
function shimHasOne(target) {
  ...
  _shimmer2.default.wrap(target, 'get', original => {
    return function batchedGetHasOne(instance) {
      ...
      const loader = loaderForModel.bySingleAttribute[this.foreignKey];
...
function createContext(sequelize) {
  ...
  function prime(results) {
    ...
    results.forEach(result => {
      const Model = result.Model ? result.Model : result.constructor;

      let loader = loaders.autogenerated.get(getLoaderKey(Model.name, Model._schema));

      if (!loader) {
        loader = loadersForModel(Model);
        loaders.autogenerated.set(getLoaderKey(Model.name, Model._schema), loader);
      }

      Object.keys(loader.bySingleAttribute).forEach(attribute => {
        loader.bySingleAttribute[attribute].prime(result.get(attribute), result);
      });
    });
  }
