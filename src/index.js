...
function loadersForModel(Model, options = {}) {
  const result = {
    bySingleAttribute: {}
  };
  ...
  sequelelize.modelManager.forEachModel(model => {
    externalAssociations = externalAssociations.concat(values(model.associations).filter(association => association.target === Model));
  });

  values(Model.associations).concat(externalAssociations).forEach(association => {
...
function shimModel(target) {
  ...
  shimmer.massWrap(target, methods(Sequelize.version).findByPk, original => {
    return function batchedFindById(id, options = {}) {
      ...
      return Promise.resolve(loader.byPrimaryKey.load(id)).then(rejectOnEmpty.bind(null, options));
    };
  });
}
