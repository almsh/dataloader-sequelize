...
function loadersForModel(Model, options) {
  ...
  result.bySingleAttribute[Model.primaryKeyAttribute] = createModelAttributeLoader(Model, Model.primaryKeyAttribute, options);
  ...
  values(Model.associations).concat(externalAssociations).forEach(association => {
    if (association.associationType === 'BelongsTo') {
      ...
      if (association.targetKey !== Target.primaryKeyAttribute) {
        result.bySingleAttribute[association.targetKey] = createModelAttributeLoader(Target, association.targetKey, options);
      }
    } else if (association.associationType === 'HasOne') {
      const Target = association.target;
      result.bySingleAttribute[association.foreignKey] = createModelAttributeLoader(Target, association.foreignKey, options);
    }
  });
  ...
  shimmer.wrap(target, 'get', original => {
    return function batchedGetBelongsTo(instance, options = {}) {
      ...
      return Promise.resolve().then(() => {
        ...
        let loaderForModel = options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.get(getLoaderKey(this.target.name, this.target._schema));

        if (!loaderForModel) {
          loaderForModel = loadersForModel(this.target, options);
          options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.set(getLoaderKey(this.target.name, this.target._schema), loaderForModel);
        }
